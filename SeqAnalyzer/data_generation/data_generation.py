...
file format:
- each line is a seperate sequence.item/letters of the sequence are seperated by commas.
...
def main(argv):
    no_space = True
    if '-no_space' in argv: #change this to yes_space
        argv.remove('-no_space')
        no_space = True
    filename = argv[1]
    noises = []
    new = 'format_'+clean(filename)
    n_col = int(argv[2])

    if os.path.isdir(filename):
        for f in os.listdir(filename):
            
            format_file(filename+'/'+f ,'format_'+clean(f) , n_col )
    else :
    ...
    while i in range(len(argv)): 
        prob = float(argv[i+1])
        noise = to_regular_string(argv[i])
    ...
    if os.path.isdir(filename):
        for f in os.listdir(filename):
            noisy_from_existing_file('format_'+clean(f),noises,no_space)
    else :
        noisy_from_existing_file('format_'+clean(filename) ,noises,no_space)
